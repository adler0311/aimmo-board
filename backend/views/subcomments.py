...
class SubcommentsView(BaseView):
    ...
    def comments(self, comment_id, **kwargs):
    ...
    def post(self, auth_token, comment_id, content, **kwargs):
    ...
    def put(self, content, subcomment_id, **kwargs):
        result = SubCommentModifyService.update(subcomment_id, content)
    ...
    @token_required
    @authorization_required
    @route('/<string:subcomment_id>', methods=['DELETE'])
    @marshal_with(ResponseSuccessSchema, code=200)
    @marshal_with(ResponseErrorSchema, code=404)
